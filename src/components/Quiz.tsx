...
import { RESULTS, MACRO_LABEL } from "@/lib/results";
...
function Result({
  ...
  const why = useMemo(() => firstSentence(r.whyHard), [r.whyHard]);
  const firstCheck = r.checkpoints[0];
  const restChecks = r.checkpoints.slice(1);
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="mt-8 animate-fadeUp [animation-delay:300ms]">
        ...
        {why.rest && (
          <Blurred>
            <p className="font-sans text-[15px] leading-relaxed text-ink/90">
              {why.rest}
            </p>
          </Blurred>
        )}
      ...
      <div className="mt-9 animate-fadeUp rounded-3xl border border-accent/40 bg-accentSoft p-6 [animation-delay:420ms]">
        <p className="font-sans text-[15px] leading-relaxed text-ink">
          ...
          ただ、本当に知ってほしいのはこの先です。
          <br />
          なぜ、あなたのタイプが痩せにくくなるのか。そして、どこを見直せば変わるのか。
          <br />
          <br />
          続きは公式LINEでお渡しします。
        </p>
